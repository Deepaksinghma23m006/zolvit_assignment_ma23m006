...
def create_docs(user_pdf_list):
    ...
    for file in user_pdf_list:
        ...
        logging.info(f"Text extracted for {file.name}.")

        # Call OpenAI API for data extraction
        llm_extracted_data = call_openai_api(pdf_text)
        ...
        if llm_extracted_data:
            ...
            if json_data:
                try:
                    ...
                    logging.info(f"JSON extracted for {file.name}: {extracted_dict}")
                    ...
                    for field, value in extracted_dict.items():
                        ...
                        row_data['Trust'] = "Trusted" if is_valid else "Untrusted"
                        metrics['field_accuracy'].setdefault(field, {'correct': 0, 'total': 0})  # Initialize if not present
                        metrics['field_accuracy'][field]['correct'] += is_valid
                        metrics['field_accuracy'][field]['total'] += 1

                    rows.append(row_data)  # Collect the row data
                    metrics['successful_extractions'] += 1
                except json.JSONDecodeError as e:
                    logging.error(f"JSON Decode Error for {file.name}: {e}")
                    st.error(f"Failed to decode JSON for {file.name}: {e}")
            else:
                logging.warning(f"No valid JSON found in API response for {file.name}.")
                st.warning(f"No valid JSON found in API response for {file.name}.")
        else:
            logging.warning(f"Data extraction failed for {file.name}.")
            st.warning(f"Data extraction failed for {file.name}.")

    # Convert the collected rows into a DataFrame
    df = pd.DataFrame(rows)

    # Initialize field accuracy metrics after DataFrame creation
    metrics['field_accuracy'] = {field: {'correct': 0, 'total': 0} for field in df.columns if field not in ['Confidence', 'Trust']}
    ...
    for field, accuracy in metrics['field_accuracy'].items():
        logging.info(f"{field}: {accuracy['correct']}/{accuracy['total']} valid extractions.")
...
if st.button("Extract Data"):
    if uploaded_files:
        extracted_data_df = create_docs(uploaded_files)
        ...
        if not extracted_data_df.empty:
            ...
            extracted_data_df.to_excel(output_file, index=False)
            st.success(f"Data successfully extracted and saved to {output_file}.")
        else:
            st.warning("No data extracted. Please check the invoices.")
    else:
        st.error("Please upload at least one PDF invoice.")
